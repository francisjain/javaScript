var countries=[
  ...
  // india:{population:123,language:hindi,currency:indianrupee}


var details={};

for(let datas of countries){
    let uid= datas.name

    if(datas.currencies){
    var c_name=datas.currencies.map(cn=>cn.name)
   }
        details[uid]={        
            population:datas.population,
            language:datas.languages.map(ln=>ln.name),
            currency:c_name        
...
//print countryname using function
function getCountryDataByName(country_name){
  return country_name in details?details[country_name]:"invalide"
}
//print currency using funtion
function getCurrencyByCountryName(country_name){
  return country_name in details?details[country_name].currency:"invalide"
}


console.log(getCountryDataByName("India"));
console.log(getCurrencyByCountryName("India"));
